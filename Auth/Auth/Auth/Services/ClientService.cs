...
namespace Auth.Services
{
    public class ClientService : IClientService
    {
        ...
        {
            var getUser = await appDbContext.Registrations.Where(_ => _.Email!.Equals(model.Email))
                .FirstOrDefaultAsync();
        ...
        private static bool VerifyUserPassword(string rawPassword, string databasePassword)
        {
            ...
            {
                if (dbPasswordHash[i + 16] != rfcPasswordHash[i]) return false;
            }
        ...
        public async Task<(bool flag, string Message)> RegisterUserAsync(Registration model)
        {
            ...
            if (string.IsNullOrEmpty(userRole.RoleName)) userRole.RoleName = "User";
